...
export default function Page() {
  return (
    <div>
      <h1>🏦 DeFi Strategist</h1>
      <p className="lead">
        <strong>Mode:</strong> <code>defi-strategist</code> · <strong>Category:</strong> Crypto / DeFi
      </p>

      <p>Analyzing DeFi opportunities, yield strategies, risk decomposition across protocols</p>

      <h2>Quick run</h2>
      <p>From the repo root, after <code>pnpm -r build</code>:</p>
      <pre><code>{`node apps/cli/dist/index.js -a defi-strategist -q "your question here"`}</code></pre>

      <h2>Requirements</h2>

      <h3>LLM provider</h3>
      <p>Recommended: <strong>anthropic (recommended), groq, gemini</strong></p>
      <p>Set the corresponding API key in <code>~/.openvesper/.env</code>:</p>
      <ul>
        <li><code>ANTHROPIC_API_KEY (or any LLM key)</code></li>
        <li><code>HELIUS_API_KEY (optional, Solana RPC)</code></li>
        <li><code>COINGECKO_API_KEY (optional)</code></li>
      </ul>

      <h3>Plugins used</h3>
      <p>This agent has access to all plugins (cross-plugin tool registry), but typically reaches for:</p>
      <p><code>defi</code>, <code>crypto</code>, <code>whale</code>, <code>onchain</code>, <code>derivatives</code></p>

      <h3>Skills it can pull in</h3>
      <p><code>defi-position-review</code>, <code>pre-trade-checklist</code>, <code>yield-risk-decomposition</code></p>

      <h2>Example sessions</h2>
      <pre><code>{`node apps/cli/dist/index.js -a defi-strategist -q "Best stablecoin yields right now under 30% APR?"`}</code></pre>
      <pre><code>{`node apps/cli/dist/index.js -a defi-strategist -q "Decompose risk of borrowing on Aave to LP on Uniswap"`}</code></pre>
      <pre><code>{`node apps/cli/dist/index.js -a defi-strategist -q "Whale outflows on Solend last 24h?"`}</code></pre>

      <h2>Scheduled mode (optional)</h2>
      <p>
        This agent ships with a heartbeat checklist in 
        <code>.agents/defi-strategist/HEARTBEAT.md</code>. The default schedule is 
        <code>0 8 * * *</code> but the heartbeat is <strong>disabled by default</strong>.
      </p>
      <p>To enable scheduled execution:</p>
      <pre><code>{`# Edit .agents/defi-strategist/HEARTBEAT.md
# Change "enabled: false" to "enabled: true"

# Then add it to your cron job state
node apps/cli/dist/index.js cron add hb-defi-strategist \
  --schedule "0 8 * * *" \
  ...
  --deliver-to "telegram:@me"`}</code></pre>
      ...
      <h2>Files</h2>
      <p>The agent's source files live under <code>.agents/defi-strategist/</code>:</p>
      <ul>

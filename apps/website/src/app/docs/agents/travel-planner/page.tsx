...
export default function Page() {
  return (
    <div>
      ...
      <p className="lead">
        <strong>Mode:</strong> <code>travel-planner</code> · <strong>Category:</strong> Lifestyle
      </p>

      <p>Trip planning, itinerary building, visa requirement lookups, booking suggestions</p>

      <h2>Quick run</h2>
      <p>From the repo root, after <code>pnpm -r build</code>:</p>
      <pre><code>{`node apps/cli/dist/index.js -a travel-planner -q "your question here"`}</code></pre>

      <h2>Requirements</h2>

      <h3>LLM provider</h3>
      <p>Recommended: <strong>anthropic, gpt-4-turbo, gemini</strong></p>
      <p>Set the corresponding API key in <code>~/.openvesper/.env</code>:</p>
      <ul>
        <li><code>ANTHROPIC_API_KEY (or other)</code></li>
        <li><code>GOOGLE_PLACES_API_KEY (optional)</code></li>
      </ul>

      <h3>Plugins used</h3>
      <p>This agent has access to all plugins (cross-plugin tool registry), but typically reaches for:</p>
      <p><code>maps</code>, <code>calendar</code></p>

      <h3>Skills it can pull in</h3>
      <p><code>weather-aware-itinerary</code></p>

      <h2>Example sessions</h2>
      <pre><code>{`node apps/cli/dist/index.js -a travel-planner -q "Plan 5 days in Istanbul for a foodie"`}</code></pre>
      <pre><code>{`node apps/cli/dist/index.js -a travel-planner -q "Visa requirements for Turkish passport to Japan?"`}</code></pre>

      <h2>Scheduled mode (optional)</h2>
      <p>
        This agent ships with a heartbeat checklist in 
        <code>.agents/travel-planner/HEARTBEAT.md</code>. The default schedule is 
        <code>0 9 * * SUN</code> but the heartbeat is <strong>disabled by default</strong>.
      </p>
      <p>To enable scheduled execution:</p>
      <pre><code>{`# Edit .agents/travel-planner/HEARTBEAT.md
# Change "enabled: false" to "enabled: true"

# Then add it to your cron job state
node apps/cli/dist/index.js cron add hb-travel-planner \
  --schedule "0 9 * * SUN" \
  ...
  --deliver-to "telegram:@me"`}</code></pre>
      ...
      <h2>Files</h2>
      <p>The agent's source files live under <code>.agents/travel-planner/</code>:</p>
      <ul>

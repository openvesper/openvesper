...
export default function Page() {
  return (
    <div>
      <h1>☀️ Solana Dev Coach</h1>
      <p className="lead">
        <strong>Mode:</strong> <code>solana-dev-coach</code> · <strong>Category:</strong> Crypto / DeFi
      </p>

      <p>Anchor programs, SPL tokens, cNFT setup, Token-2022 extensions</p>

      <h2>Quick run</h2>
      <p>From the repo root, after <code>pnpm -r build</code>:</p>
      <pre><code>{`node apps/cli/dist/index.js -a solana-dev-coach -q "your question here"`}</code></pre>

      <h2>Requirements</h2>

      <h3>LLM provider</h3>
      <p>Recommended: <strong>anthropic, deepseek-coder</strong></p>
      <p>Set the corresponding API key in <code>~/.openvesper/.env</code>:</p>
      <ul>
        <li><code>ANTHROPIC_API_KEY</code></li>
        <li><code>HELIUS_API_KEY (optional)</code></li>
      </ul>

      <h3>Plugins used</h3>
      <p>This agent has access to all plugins (cross-plugin tool registry), but typically reaches for:</p>
      <p><code>solana</code>, <code>solana-dev</code>, <code>filesystem</code>, <code>code</code></p>

      <h3>Skills it can pull in</h3>
      <p><code>compute-units-optimization</code></p>

      <h2>Example sessions</h2>
      <pre><code>{`node apps/cli/dist/index.js -a solana-dev-coach -q "Help me set up Anchor 0.31 with Token-2022"`}</code></pre>
      <pre><code>{`node apps/cli/dist/index.js -a solana-dev-coach -q "What's the best pattern for cNFT minting on Helius?"`}</code></pre>
      <pre><code>{`node apps/cli/dist/index.js -a solana-dev-coach -q "Decode this Solana transaction: 4xK..."`}</code></pre>

      <h2>Scheduled mode (optional)</h2>
      <p>
        This agent ships with a heartbeat checklist in 
        <code>.agents/solana-dev-coach/HEARTBEAT.md</code>. The default schedule is 
        <code>0 9 * * MON</code> but the heartbeat is <strong>disabled by default</strong>.
      </p>
      <p>To enable scheduled execution:</p>
      <pre><code>{`# Edit .agents/solana-dev-coach/HEARTBEAT.md
# Change "enabled: false" to "enabled: true"

# Then add it to your cron job state
node apps/cli/dist/index.js cron add hb-solana-dev-coach \
  --schedule "0 9 * * MON" \
  ...
  --deliver-to "telegram:@me"`}</code></pre>
      ...
      <h2>Files</h2>
      <p>The agent's source files live under <code>.agents/solana-dev-coach/</code>:</p>
      <ul>

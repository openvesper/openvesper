...
export default function Page() {
  return (
    <div>
      ...
      <p className="lead">
        <strong>Mode:</strong> <code>legal-assistant</code> · <strong>Category:</strong> Productivity
      </p>

      <p>Contract review, deadline tracking, summarizing legal docs</p>

      <h2>Quick run</h2>
      <p>From the repo root, after <code>pnpm -r build</code>:</p>
      <pre><code>{`node apps/cli/dist/index.js -a legal-assistant -q "your question here"`}</code></pre>

      <h2>Requirements</h2>

      <h3>LLM provider</h3>
      <p>Recommended: <strong>anthropic claude-opus (best for nuance)</strong></p>
      ...
      <p><code>filesystem</code>, <code>calendar</code></p>

      <h3>Skills it can pull in</h3>
      <p><code>contract-red-flag-scan</code></p>

      <h2>Example sessions</h2>
      <pre><code>{`node apps/cli/dist/index.js -a legal-assistant -q "Summarize contracts/2026/vendor-agreement.pdf"`}</code></pre>
      <pre><code>{`node apps/cli/dist/index.js -a legal-assistant -q "What contract deadlines do I have in the next 30 days?"`}</code></pre>

      <h2>Scheduled mode (optional)</h2>
      <p>
        This agent ships with a heartbeat checklist in 
        <code>.agents/legal-assistant/HEARTBEAT.md</code>. The default schedule is 
        <code>0 9 * * MON</code> but the heartbeat is <strong>disabled by default</strong>.
      </p>
      <p>To enable scheduled execution:</p>
      <pre><code>{`# Edit .agents/legal-assistant/HEARTBEAT.md
# Change "enabled: false" to "enabled: true"

# Then add it to your cron job state
node apps/cli/dist/index.js cron add hb-legal-assistant \
  --schedule "0 9 * * MON" \
  ...
  --deliver-to "telegram:@me"`}</code></pre>
      ...
      <h2>Files</h2>
      <p>The agent's source files live under <code>.agents/legal-assistant/</code>:</p>
      <ul>
